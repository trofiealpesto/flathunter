...
const envSchema = z.object({
  ...
  CEREBRAS_API_BASE_URL: z.string().url().default("https://api.cerebras.ai/v1"),
  GEMINI_CLASSIFIER_MAX_PER_RUN: z.coerce.number().int().nonnegative().default(40),
  GEMINI_CLASSIFIER_FALLBACK_MAX_PER_RUN: z.coerce.number().int().nonnegative().default(1),
  GEMINI_CLASSIFIER_MIN_DELAY_MS: z.coerce.number().int().nonnegative().default(2_200),
